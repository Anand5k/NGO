...
app.post('/ngo/assign', async (req, res) => {
  ...
  try {
    ...
    // Fetch emails of users whose entries were updated
    const emailQuery = `
      SELECT email, type, location 
      FROM registry 
      WHERE id = ANY($1::text[])
      AND ngo = $2
    `;
    const usersResult = await db.query(emailQuery, [selectedIds, name]);

    // Send emails to users
    const userEmails = usersResult.rows;
    for (let user of userEmails) {
      await sendAssignmentConfirmation(user.email, user.type, user.location, name);
    }

    res.json({ message: 'Entries assigned and users notified successfully' });
  } catch (error) {
    console.error('Error assigning NGO:', error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
async function sendAssignmentConfirmation(recipient, type, location, ngoName) {
  let transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: "restor.monument@gmail.com",
      pass: "ewpp jyen kxrv akce"
    }
  });
  ...
  let mailOptions = {
    ...
    to: recipient,
    subject: `Your Restoration Request has been Assigned`,
    text: `Hello,\n\nYour application for restoring "${type}" deterioration at "${location}" has been assigned to the NGO: ${ngoName}.\nWe shall keep you updated on the status of restoration.\n\nThank you for contributing to heritage restoration.\n\nBest Regards,\nNGO Management System`
  };

  try {
    let info = await transporter.sendMail(mailOptions);
    console.log(`📩 Confirmation email sent to user: ${recipient}`);
  } catch (error) {
...
app.post('/ngo/update-status', async (req, res) => {
  ...
  try {
    ...
    const checkResult = await db.query(
      `SELECT email, type, location FROM registry WHERE id = $1 AND ngo = $2 AND status != 'completed'`,
      [id, name]
    );
    ...
    const { email: userEmail, type, location } = checkResult.rows[0];
    ...
    // Send email to user with new status
    await sendStatusUpdateEmail(userEmail, type, location, newStatus);

    res.json({ message: 'Status updated and user notified successfully' });
  } catch (error) {
    console.error('Error updating status:', error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
async function sendStatusUpdateEmail(recipient, type, location, status) {
  let transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: "restor.monument@gmail.com",
  ...
  let mailOptions = {
    from: "restor.monument@gmail.com",
    to: recipient,
    subject: `Update on Your ${type} Restoration Application`,
    text: `Hello,\n\nThis is to inform you that the status of your application for restoring "${type}" at "${location}" has been updated to: "${status}".\n\nThank you for your patience and contribution to heritage preservation.\n\nBest Regards,\nNGO Management System`
  };

  try {
    let info = await transporter.sendMail(mailOptions);
    console.log(`📩 Status update email sent to user: ${recipient}`);
  } catch (error) {
...
app.post('/ngo/new-application', upload.single("image"), async (req, res) => {
    try {
        ...
        await sendConfirmationToUser(email, type, location);
...
async function sendConfirmationToUser(recipient, type, location) {
  let transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: "restor.monument@gmail.com",
  ...
  let mailOptions = {
    from: "restor.monument@gmail.com",
    to: recipient,
    subject: "Your Restoration Application Has Been Received",
    text: `Hello,\n\nThank you for submitting your application for restoring "${type}" at "${location}".\n\nOur team is reviewing your request and matching it with the appropriate NGO. You will be notified once it is assigned.\n\nBest Regards,\nNGO Management System`
  };
...
app.post('/ngo/delete-entry', async (req, res) => {
  const { id } = req.body;

  try {
    // Fetch the entry before deletion
    const entryResult = await db.query(
      `SELECT email, type, location FROM registry WHERE id = $1`,
      [id]
    );

    if (entryResult.rows.length === 0) {
      return res.status(404).json({ error: 'No such entry found in registry' });
    }

    const { email, type, location } = entryResult.rows[0];

    // Proceed to delete
    const result = await db.query('DELETE FROM registry WHERE id = $1', [id]);

    // Send email to the user
    await sendDeletionNotification(email, type, location);

    res.status(200).json({ message: 'Entry deleted and user notified successfully' });
  } catch (error) {
    console.error('Error deleting entry from registry:', error);
    res.status(500).json({ error: 'Failed to delete entry from registry' });
  }
});
async function sendDeletionNotification(recipient, type, location) {
  let transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: "restor.monument@gmail.com",
  ...
  let mailOptions = {
    from: "restor.monument@gmail.com",
    to: recipient,
    subject: `Your Restoration Application Has Been Removed`,
    text: `Hello,\n\nYour application for restoring "${type}" at "${location}" has been removed since it has been found invalid.\nIf this was a mistake or you have any questions, please feel free to re-apply or contact us.\n\nThank you,\nNGO Management System`
  };

  try {
    let info = await transporter.sendMail(mailOptions);
    console.log(`📩 Deletion email sent to user: ${recipient}`);
  } catch (error) {
